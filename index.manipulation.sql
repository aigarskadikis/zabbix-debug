...
mysql zabbix -sN -e 'SELECT DISTINCT hosts.hostid,GROUP_CONCAT(items.itemid) FROM items JOIN hosts ON (hosts.hostid=items.hostid) WHERE items.value_type=1 AND hosts.status IN (0,1) GROUP BY items.hostid;' | \
sed 's|^|PARTITION hostid_|;s|\t| VALUES IN (|;s|$|),|'

tr '\t' ' VALUES IN ('

sed "s|^|PARTITION hostid|"
sed 's/\t/,/g'
